...
serve(async (req) => {
  try {
    ...
    const { data: macros } = await supabase.from('task_macros').select('*');
    ...
    if (activeCall && activeCall.name === "search_app_directory") {
      ...
      const secondPayload = {
        ...
        contents: [
          ...
          { role: "function", parts: [{ functionResponse: { name: "search_app_directory", response: { results: apps } } }] }
        ]
      };
